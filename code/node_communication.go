...
func (srh *SybilResistanceHandler) handleInactivePacket(packet InactiveNode, peer string){
	msg := &GossipPacket{InNode:&packet, DigitalSign : srh.signDocument(srh.createHash(packet.IPAddr))}
	srh.broadcastInactivePacketToOthers(msg,peer)
}
...
func (srh *SybilResistanceHandler) broadcastInactivePacketToOthers(msg *GossipPacket, peerAddr string){
	for _,peer := range(srh.ps.peers){
		if strings.Compare(peer.address.String(), peerAddr) != 0{
			fmt.Println("Send Inactive Packet to "+peer.address.String())
			srh.ps.send(msg, peer.address)
		}
	}
